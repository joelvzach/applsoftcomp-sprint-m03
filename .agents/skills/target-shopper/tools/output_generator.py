...
def generate_output_summary(
    ...
    filename = f"output_{timestamp.strftime('%Y%m%d_%H%M%S')}.md"
...
"""
    ...
    for i, item in enumerate(items_in_order, 1):
        content += f"{i}. **{item['name']}** - Aisle {item['aisle']} (${item['price']:.2f})\n"
...
"""
    ...
    if html_link or svg_link:
        ...
        if svg_link:
            content += f"- [📄 Static Route SVG]({svg_link}) - High-quality vector image\n"
        content += "\n"
