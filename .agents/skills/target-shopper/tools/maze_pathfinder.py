...
def heuristic(
    cell1: Tuple[int, int], cell2: Tuple[int, int], method: str = "euclidean"
) -> float:
    """
...
def get_neighbors(
    cell: Tuple[int, int], grid: np.ndarray
            ...
            if neighbor not in all_nodes or tentative_g < all_nodes[neighbor].g_score:
                if neighbor not in all_nodes:
                    new_node = PathNode(
                        ...
                        g_score=tentative_g,
                        f_score=tentative_g
                        + heuristic(neighbor, goal, heuristic_method),
                        parent=current,
                        direction=direction,
                    )
                    ...
                    old_node = all_nodes[neighbor]
                    all_nodes[neighbor] = PathNode(
                        cell=neighbor,
                        g_score=tentative_g,
                        f_score=tentative_g
                        + heuristic(neighbor, goal, heuristic_method),
                    ...
                    # Remove old node from open_set and add updated one
                    open_set = [n for n in open_set if n.cell != neighbor]
                    heapq.heappush(open_set, all_nodes[neighbor])
...
def snap_to_walkable(
    maze, cell: Tuple[int, int], max_distance: int = 5
) -> Tuple[int, int]:
    """
...
def find_maze_path(
    ...
    for i in range(len(all_points) - 1):
        segment = astar_search(
            maze.grid, all_points[i], all_points[i + 1], heuristic_method
        )
