    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if title and "Target" in title and "Store" in title:
                ...
                return {"name": name, "url": store_url, "store_id": store_id}
...
def search_by_state_directory(
    state: str, city: str = None, headless: bool = True
) -> List[Dict[str, str]]:
    """
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            state_url = (
                f"https://www.target.com/store-locator/store-directory/{state_slug}"
            )
    ...
    if city and stores:
        ...
        filtered = [s for s in stores if city_lower in s["name"].lower()]
        if filtered:
            ...
            if city_lower in ["new york city", "nyc"]:
                ...
                if borough_filtered:
                    ...
                    print(
                        f"  → No stores found in {city}, showing all {len(stores)} stores in {state}"
                    )
            else:
                print(
                    f"  → No stores found in {city}, showing all {len(stores)} stores in {state}"
...
def get_recent_stores(max_age_days: int = 30, limit: int = 5) -> List[Dict]:
    ...
    while current != current.parent:
        if (current / ".git").exists():
            cache_file = current / "project" / ".store_cache.json"
            break
...
def save_store_to_cache(store: Dict):
    ...
    try:
        ...
        cache["stores"] = [
            s for s in cache["stores"] if s["store"]["store_id"] != store["store_id"]
        ]

        # Add new entry with timestamp
        cache["stores"].append(
            {"store": store, "last_used": datetime.now().isoformat()}
        )
            ...
            if store_buttons:
                ...
                try:
                    ...
                    search_input = modal.locator(
                        'input[name="zip-code-city-or-state"]'
                    ).first
...
def find_store(
    ...
    if recent_stores:
        ...
        for recent in recent_stores:
            if (
                query_lower in recent["name"].lower()
                or query_lower in recent.get("address", "").lower()
            ):
    ...
    if " " in store_name:
        ...
        if city_stores and (
            not stores or (len(stores) >= 10 and len(city_stores) < 10)
        ):
...
def save_store_preference(store: Dict[str, str], preferences_path: str = None) -> bool:
    ...
    try:
        ...
        store_url_added = False

        for line in lines:
            if line.startswith("## Store Location"):
                in_store_section = True
                new_lines.append(line)
            elif line.startswith("## ") and not line.startswith("## Store Location"):
                # Entering a new section - make sure we added the URL
                if in_store_section and not store_url_added:
                    new_lines.append(f"- Store URL: {store['url']}")
                    store_url_added = True
                in_store_section = False
                new_lines.append(line)
            elif in_store_section and line.startswith("- Default store:"):
                new_lines.append(
                    f"- Default store: {store['name']} (ID: {store['store_id']})"
                )
                store_url_added = True
            elif in_store_section and line.startswith("- Store URL:"):
                # Skip old Store URL lines - we'll add the new one
                continue
            elif in_store_section and line.startswith("-"):
                # Keep other lines in store section
                new_lines.append(line)
            else:
                new_lines.append(line)

        # If we were still in store section at EOF, add the URL
        if in_store_section and not store_url_added:
            new_lines.append(f"- Store URL: {store['url']}")
