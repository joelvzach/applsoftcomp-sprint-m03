...
def write_progress(
    item: str, status: str, aisle: str = None, progress_path: str = None
):
...
def search_item_subagent(args):
    ...
    Args: (item_name, store_url, headless, progress_path, trace_dir)
    """
    item_name, store_url, headless, progress_path, trace_dir = args

    write_progress(item_name, "searching", progress_path=progress_path)

    result = search_item(item_name, store_url, headless, trace_dir=trace_dir)
...
def parallel_search(
    ...
    Args:
        ...
        progress_path: Path to progress log file
        trace_dir: Directory to save Playwright traces
    """
    if not items:
        return []
    ...
    with open(progress_path, "w") as f:
        f.write(
            f"# Item Search Progress\n# Store: {store_url}\n# Items: {len(items)}\n\n"
        )

    workers = min(max_workers, len(items))
    tasks = [(item, store_url, headless, progress_path, trace_dir) for item in items]
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(search_item_subagent, task): task[0] for task in tasks
        }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        ...
        print(
            "Example: python parallel_search.py https://www.target.com/sl/store-1056/1056 eggs milk bread butter cheese"
        )
        sys.exit(1)
